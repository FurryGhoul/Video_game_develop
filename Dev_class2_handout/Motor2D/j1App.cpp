...
bool j1App::Awake()
{
	...
	result = document.load_file("config.xml");

	if (result)
	{
		LOG("Loaded %s",result.description());
		node = document.child("config").child("name");
	}
	else
	{
		LOG("And you fail %s", result.description());
	}
...
xml_node* j1App::GetNode(p2SString str) {
	xml_node Nodecheck = node.child("config").child("str");
	return &Nodecheck;
}
